...
def extract_arch_version(arch_string: str):
...
def _check_capability():
    ...
    if torch.version.cuda is not None:  # on ROCm we don't want this check
        ...
        for d in range(device_count()):
            ...
            min_arch = min(
                (extract_arch_version(arch) for arch in torch.cuda.get_arch_list()),
                default=35,
            )
...
"""
    ...
    if len(arch_list) == 0:
        return
    supported_sm = [extract_arch_version(arch) for arch in arch_list if "sm_" in arch]
